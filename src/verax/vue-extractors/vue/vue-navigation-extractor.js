...
import { HARD_EXCLUSIONS } from '../../learn/scan-roots.js';
...
export function extractVueNavigationPromises(programOrProjectRoot, maybeProjectRoot) {
  ...
  try {
    const vueFilesFromGlob = globSync('**/*.vue', { 
      cwd: projectRoot, 
      absolute: true, 
      nodir: true,
      ignore: HARD_EXCLUSIONS 
    });
    vueFilesFromGlob.sort((a, b) => a.localeCompare(b, 'en'));
  ...
  const orderedVueFiles = Array.from(new Set(vueFiles.map(f => resolve(f)))).sort((a, b) => a.localeCompare(b, 'en'));
  for (const filePath of orderedVueFiles) {
