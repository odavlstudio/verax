...
function classifyExpectation(expectation, observation) {
  ...
  const hasStrongSignals = Boolean(
    observation?.signals?.navigationChanged ||
    observation?.signals?.meaningfulDomChange ||
    observation?.signals?.feedbackSeen ||
    observation?.signals?.networkActivity ||
    observation?.signals?.correlatedNetworkActivity ||
    evidenceSignals.hasDomChange ||
    evidenceSignals.hasNetworkLogs
  );
  const hasAnyEvidence = hasStrongSignals;
