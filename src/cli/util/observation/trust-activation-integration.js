...
export async function completePhase6A(artifactDir) {
  try {
    ...
    const allFiles = readdirSync(stagingDir).sort((a, b) => a.localeCompare(b, 'en'));
...
export async function rollbackPhase6A(artifactDir, error) {
  ...
  // @ts-expect-error - readFileSync with encoding returns string
    ...
    const stagingDir = getStagingPath(artifactDir, '').replace(/\/$/, '');
    if (existsSync(stagingDir)) {
      const files = readdirSync(stagingDir).sort((a, b) => a.localeCompare(b, 'en'));
...
export function discoverArtifacts(dir) {
  ...
  try {
    return readdirSync(dir).sort((a, b) => a.localeCompare(b, 'en')).filter(f => f.endsWith('.json'));
  } catch (error) {
    return [];
  }
}
