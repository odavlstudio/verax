 ...
 * 
 * Contract tests for evidence-based stability measurement and SLA classification.
 */
...
const testRoot = resolve(dirname(import.meta.url.replace('file://', '')), '../../tmp/test-stability');
...
async function createDeterministicRun(projectRoot, runId) {
  ...
  const summary = {
    meta: { runId, generatedAt: getTimeProvider().iso() },
    analysis: {
      state: 'COMPLETE',
...
async function testStableClassification() {
  ...
  assert.strictEqual(stability1.meta.state, 'COMPLETE', 'Run 1 should be complete');
  assert.strictEqual(stability2.meta.state, 'COMPLETE', 'Run 2 should be complete');
  assert.strictEqual(stability3.meta.state, 'COMPLETE', 'Run 3 should be complete');
